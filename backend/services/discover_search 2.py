...
def _google_custom_search(query: str, limit: int = 12) -> List[Dict[str, Any]]:
    settings = get_settings()
    key = getattr(settings, "google_custom_search_api_key", "")
    ...
    request = Request(
        ...
        headers={"User-Agent": "Mozilla/5.0 LuxeLookAI/1.0"},
    )
    try:
        ...
        if body:
            try:
                ...
                error_block = parsed.get("error") or {}
                message = error_block.get("message") or ""
                status = error_block.get("status") or ""
                detail = f"{status}: {message}".strip(": ").strip()
            except Exception:
                detail = body[:400].strip()

        if detail:
            raise RuntimeError(f"Google Custom Search {exc.code}: {detail}") from exc
        raise RuntimeError(f"Google Custom Search {exc.code}: {exc.reason}") from exc

    results: List[Dict[str, Any]] = []
    for item in data.get("items", [])[:limit]:
        image_info = item.get("image") or {}
        image_url = item.get("link") or image_info.get("contextLink") or image_info.get("thumbnailLink")
        thumbnail_url = image_info.get("thumbnailLink") or image_url
        results.append({
            "source_url": image_info.get("contextLink") or item.get("link") or image_url,
            "image_url": image_url,
            "thumbnail_url": thumbnail_url,
            "title": item.get("title") or "Fashion edit",
            "source_domain": urlsplit(image_info.get("contextLink") or item.get("link") or "").netloc or "google",
        })
    return results
...
def resolve_discover_search_provider() -> str:
    ...
    if provider in {"", "auto"}:
        if getattr(settings, "google_custom_search_api_key", "") and getattr(settings, "google_custom_search_cx", ""):
...
def search_discover_images(query: str, limit: int = 12) -> Tuple[str, List[Dict[str, Any]]]:
    ...
    if provider == "google_custom_search":
        return provider, _google_custom_search(query, limit=limit)
